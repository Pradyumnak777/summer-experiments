...
try:
    from diffusers.utils import pt_to_pil as _diffusers_pt_to_pil
except Exception:
    _diffusers_pt_to_pil = None


def pt_to_pil(images):
    if _diffusers_pt_to_pil is not None:
        return _diffusers_pt_to_pil(images)
    if isinstance(images, list):
        return [pt_to_pil(image)[0] if isinstance(image, torch.Tensor) else image for image in images]
    if not isinstance(images, torch.Tensor):
        raise TypeError(f"Unsupported type for pt_to_pil: {type(images)}")
    if images.ndim == 3:
...
class EditStableDiffusion(object):
    def __init__(self, args):
        ...
        if hasattr(args, 'lora_path') and args.lora_path:
            ...
            self.pipe.load_lora_weights(args.lora_path)
            self.pipe.fuse_lora()
            print('LoRA fused into UNet.')

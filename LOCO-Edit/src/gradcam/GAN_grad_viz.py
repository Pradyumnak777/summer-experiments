...
def jacobian_gen(G, w_invert):
    ...
    if w_invert.ndim == 2:
        ...
        v[0, DIM_TO_PROBE] = 1.0
    else:
        # Extended W+ space: [1, num_ws, 512]
        # Broadening across ALL layers for dimension i:
        v[0, :, DIM_TO_PROBE] = 1.0
...
if __name__ == "__main__":
    ...
    # Invert image into W-space
    w_steps = project(G, D, target=x_target, num_steps=NUM_STEPS, device=DEVICE, verbose=False)
    w_invert = w_steps[-1].unsqueeze(0)
    w_invert = w_invert.detach().requires_grad_(True)
    
    jvp_flat, img_flat = jacobian_gen(G, w_invert)
    C, H, W = x.shape

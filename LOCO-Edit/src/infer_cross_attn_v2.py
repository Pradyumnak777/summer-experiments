...
SAMPLE_INDICES = [799]   #dataset indices to run
SEED           = 0
OUT_DIR        = f"cross_attn/AtoB/attn_maps_real_noised_startT{START_T}"
...
def generate_and_collect(src_img, tgt_img):
    ...
    return latents, agg, per_token, src_side, tgt_side
...
def save_sample(sample_dir, chA, chB_real, chB_gen, agg, per_token, src_side, tgt_side):
    ...
    for y in range(src_side):
        for x in range(src_side):
            ...
            ax = axes[y, x]
            ax.imshow(overlay_on(chB_gen, m))
            ax.set_xticks([]); ax.set_yticks([])
    fig.suptitle(f"per-chA-token attn over chB, summed over full trajectory, "
                 f"one cell per chA position ({src_side}x{src_side}={src_side*src_side} total)")
    fig.tight_layout()
    ...
    for y in range(src_side):
        for x in range(src_side):
            ...
            axes[1].imshow(overlay_on(chB_gen, m))
            axes[1].set_title("chB attends to this token\n(summed over full trajectory)"); axes[1].axis("off")
            fig.tight_layout()
...
for i in SAMPLE_INDICES:
    ...
    recon, agg, per_token, src_side, tgt_side = generate_and_collect(src_img, tgt_img)
    chA = to_img(src_img)
    chB_real = to_img(tgt_img)
    chB_gen = to_img(recon)

    sample_dir = f"{OUT_DIR}/sample_{i}"
    save_sample(sample_dir, chA, chB_real, chB_gen, agg, per_token, src_side, tgt_side)
    ...
    print(f"sample idx {i} -> {sample_dir}/  "
          ...
          f"grid.png + {src_side*src_side} individual files)")

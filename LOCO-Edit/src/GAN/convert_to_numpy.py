...
def main():
    ...
    for a_path, b_path in zip(chA, chB):
        ...
        mask = Image.open(mask_path).convert('L').resize((SIZE, SIZE), resample=Image.NEAREST)
        ...
        a_masked = np.where(mask, a, 0).astype(np.uint8)  
        b_masked = np.where(mask, b, 0).astype(np.uint8)
